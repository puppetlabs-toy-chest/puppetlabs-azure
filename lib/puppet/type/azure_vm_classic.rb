...
Puppet::Type.newtype(:azure_vm_classic) do
  ...
  validate do
    if self[:password] && self[:private_key_file]
      raise 'You can only provide either a password or a private_key_file for an Azure VM'
    end
    if self[:subnet] && !self[:virtual_network]
      raise 'When specifying a subnet you must also specify a virtual network'
    end
    if self[:virtual_network] && self[:affinity_group]
      raise 'You can only provide either a virtual_network or an affinity_group for an Azure VM'
    end
    required_properties = [
      :location
    ]
    required_properties.each do |property|
      ...
      if self[property].nil? && provider.send(property) == :absent
  ...
  newproperty(:endpoints, array_matching: :all) do
    ...
    validate do |value|
      raise "endpoints should be an Array of Hashes, but contains a #{value.class}" unless value.is_a? Hash
      stringified_value = {}
      value.each { |k, v| stringified_value[k.to_s] = v }
      required = %w(name public_port local_port protocol)
      missing = required - stringified_value.keys.map(&:to_s)
      unless missing.empty?
        raise "an endpoint is missing the following keys: #{missing.join(',')}"
      end
      %w(protocol load_balancer_name).each do |key|
        if stringified_value.keys.include? key
          raise "#{key} for endpoints should be a String" unless stringified_value[key].is_a? String
        end
      end
      %w(public_port local_port).each do |key|
        if stringified_value.keys.include? key
          raise "#{key} for endpoints should be an Integer" unless stringified_value[key].to_i.to_s == stringified_value[key].to_s
        end
      end
      ['direct_server_return'].each do |key|
        if stringified_value.keys.include? key
          raise "#{key} for endpoints should be a Boolean, not #{stringified_value[key].inspect}" unless %w(true false).include?(stringified_value[key].to_s)
        end
      end
      if stringified_value.keys.include? 'load_balancer'
        raise 'load_balancer for endpoints should be a Hash' unless stringified_value['load_balancer'].is_a? Hash
        raise 'having a load_balancer requires a load_balancer_name' unless stringified_value.keys.include? 'load_balancer_name'
      end
    end

    munge do |value|
      value = value.each_with_object({}) { |(k, v), memo| memo[k.to_sym] = v }
