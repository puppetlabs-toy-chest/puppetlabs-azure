...
Puppet::Type.type(:azure_resource_template).provide(:arm, parent: PuppetX::Puppetlabs::Azure::ProviderArm) do
  ...
  def self.prefetch(resources)
    instances.each do |prov|
      if resource = (resources.find { |k, _v| k.casecmp(prov.name).zero? } || [])[1] # rubocop:disable Lint/AssignmentInCondition
  ...
  def flush
    create_resource_template(create_hash) if @property_hash[:ensure] != :absent
  end
    ...
    if resource[:source]
      if resource[:source] !~ %r{^(puppet|https?)://}
        ...
        if text
          ...
          raise "source appears to be a local path but does not exist: #{source}"
        end
      end
    else
      resource[:content]
    ...
    if resource[:params_source]
      if resource[:params_source] !~ %r{^(puppet|https?)://}
        ...
        if text
          ...
          raise "params_source appears to be a local path but does not exist: #{params_source}"
        end
      end
    else
      resource[:params]
