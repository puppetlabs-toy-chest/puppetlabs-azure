...
describe 'azure_vm_classic when creating a machine' do
  ...
  describe 'with all available properties' do
    before(:all) do
      @custom_data_file = is_windows? ? '/cygdrive/c/needle.txt' : '/tmp/needle'
      ...
      @config = {
        ...
        optional: {
          ...
          size: 'Small',
          # deployment: "CLOUD-DN-#{@tag_seed}", # CLOUD-1611 raised
          cloud_service: SPEC_CLOUD_SERVICE,
          # data_disk_size_gb: 53, # CLOUD-1592 raised.
    ...
    it 'should be in the correct storage account' do
      ...
      expect(storage_account.label).to eq(@config[:optional][:storage_account])
    end
    ...
    context 'when stopping the created vm' do
      before(:all) do
        config = { 
          :name => @name,
          :ensure => 'stopped',
